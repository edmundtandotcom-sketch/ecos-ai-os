# Second Property Ladder — Landing Page Wireframes v1.0 (Phase 1B)

Version: v1.0
Status: APPROVED MASTER — approved by Coach Edmund 2026-07-16
Date: 2026-07-16
Owner: Coach Edmund (Programme Owner)
Supersedes: none — first Phase 1B landing-page wireframe master. A prior research pass produced an equivalent Door A / Door B wireframe draft with the same open decisions; that draft's exact text was not available to reconstruct from, so this document rebuilds equivalent content directly from the two approved Phase 1 masters below, with every claim/CTA/proof category cited inline.
Sources: `10_MARKETING_FOUNDATION_v1.0.md` (APPROVED MASTER, esp. §§1, 2, 4, 5, 6, 7, 8, 9, 10, 11) · `11_MINIMUM_SELECTION_DOCTRINE_v1.0.md` (APPROVED MASTER, esp. §§2, 3, 4, 10) · `14_SECOND_PROPERTY_READINESS_CHECK_v1.0.md` (APPROVED MASTER — referenced only for the three public route names and the Check's public content boundary; its internal question bank, scoring and GHL fields are explicitly not reused here) · Edmund's rulings R-1–R-7 (2026-07-16, this session)
Related: `10_MARKETING_FOUNDATION_v1.0.md` · `11_MINIMUM_SELECTION_DOCTRINE_v1.0.md` · `14_SECOND_PROPERTY_READINESS_CHECK_v1.0.md` · `12_UNRESOLVED_DECISIONS.md` · `13_CHANGELOG.md`

**Rule:** where this master conflicts with `10_MARKETING_FOUNDATION_v1.0.md` or `11_MINIMUM_SELECTION_DOCTRINE_v1.0.md`, those two masters win. Where it conflicts with `14_SECOND_PROPERTY_READINESS_CHECK_v1.0.md` on what the Check makes public, that document's public/backend boundary (its §2, inherited from Doctrine §2) wins.

**SCOPE BOUNDARY:** this is copy direction and wireframe structure only — real sample headlines, body copy, section order, CTA placement, and design direction in prose. It is not pixel design, not HTML/CSS/JS, not the actual page build, and does not wire any ad destination URL. A later Phase 1B implementation session builds from this document. No claim, CTA, or proof category below goes beyond what `10_MARKETING_FOUNDATION_v1.0.md` and `11_MINIMUM_SELECTION_DOCTRINE_v1.0.md` already approved.
...
This page is not the sales pitch and not the diagnostic — it is the bridge between an ad and the Second Property Readiness Check™ (`10_MARKETING_FOUNDATION_v1.0.md` §10 offer bridge: *Ad or content → Second Property Readiness Check™ → result → appropriate CTA → Second Property Strategy Session™*). Its one job: make the specific reader see their own situation reflected back at them, let the belief reframe and proof do the convincing, and land on one unmistakably clear next step — never a menu of options.

Every headline below is adapted from a locked angle hook in `10_MARKETING_FOUNDATION_v1.0.md` §7 — none is invented. Every process claim traces to the Doctrine's public selection process (§3) or public/backend boundary (§2). Every proof reference traces to the Foundation's proof architecture (§8) and the Doctrine's illustrative case patterns (§10), built as anonymised composites per R-2/R-5 — never a named testimonial, never an aggregate statistic (blocked until the Rejection Ledger produces a measured figure, Doctrine §9, Foundation §11).
...
- **Section 3 — Both names, one trap (A2):** *"Both names on one property can feel like the safe, responsible choice — until you want the next one."* Body: explains that ownership structure is a route decision, not an afterthought, and previews that the Route Map addresses "what your current name structure actually allows" before any property search begins.
...
- The Readiness Check's internal question bank, gate logic, or scoring mechanic (`14_SECOND_PROPERTY_READINESS_CHECK_v1.0.md` §§4, 5, 6, 7, 11 — internal/backend).
...
> Headline: *Waiting without knowing what you're waiting for is not a strategy.*
> Body: "Your property may have increased in value — but if it hasn't created your next move, the gain may still be sitting idle. (Foundation §3.) This isn't a push to sell today. It's worth knowing plainly: what does waiting change for your MOP or SSD position, your names, your family's timeline?" *Any specific figure used here (MOP/SSD rules, interest-rate assumptions, ABSD rates) carries "DRAFT — pending platform verification" until fact-checked at implementation time, per constitution §11 and the same rule applied in `14_SECOND_PROPERTY_READINESS_CHECK_v1.0.md` §9.*
...
- The Readiness Check's internal question bank, gate logic, or scoring mechanic (`14_SECOND_PROPERTY_READINESS_CHECK_v1.0.md` §§4, 5, 6, 7, 11).
- The public 90% disqualification figure as a stated percentage (Doctrine §9).
- The retired route-naming scheme "Hold Two / Trade Up First / Prepare Before Moving" (superseded per Foundation §5, U-3).
...
- Everything already out of scope in `10_MARKETING_FOUNDATION_v1.0.md` §13 and `14_SECOND_PROPERTY_READINESS_CHECK_v1.0.md` §13 (REI Method operational rebuild, Year-4 Exit Review Doctrine, the 8-stage buyer consult journey beyond Self-Assessment, the $5M+ segment as a dedicated door, SRX X-Value licensing, `spl_*` data migration).

---

## 5. Next step

Per `10_MARKETING_FOUNDATION_v1.0.md` §14, this closes the Phase 1B wireframe layer. Phase 1C (first creative test packs — three angles per door, multiple treatments per angle) is next, and depends on this document's §4.3 dynamic-hero technical dependency being wired at build time. Phase 1D (proof system) follows.
