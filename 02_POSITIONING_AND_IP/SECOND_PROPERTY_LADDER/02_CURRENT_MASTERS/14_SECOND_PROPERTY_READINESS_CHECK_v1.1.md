# Second Property Ladder — Second Property Readiness Check™ v1.1 (Phase 1A Strategy)

Version: v1.1
Status: APPROVED MASTER — approved by Coach Edmund 2026-07-16
Date: 2026-07-19
Owner: Coach Edmund (Programme Owner)
Supersedes: v1.0 — aligns authority, booked-session naming, current master filenames and diagnostic-recovery paths after the 2026-07-19 Positioning & IP audit.
Sources: `10_MARKETING_FOUNDATION_v1.1.md`; `11_MINIMUM_SELECTION_DOCTRINE_v1.1.md`; `12_DECISION_RECONCILIATION.md`; `01_PHASE_1_MARKETING_FOUNDATION\90_SOURCE_MARKDOWN\02_SECOND_LADDER_NEW_EXECUTION.md`; archived `Which_Move_Are_You_On_Diagnostic_v1.2`; diagnostic recovery snapshot in `_BACKUPS\ECOS_BACKUPS`; Decision Memory v3.15; Edmund's R-1–R-5 rulings.
Related: `10_MARKETING_FOUNDATION_v1.1.md` · `11_MINIMUM_SELECTION_DOCTRINE_v1.1.md` · `12_DECISION_RECONCILIATION.md` · `13_CHANGELOG.md`

**Authority rule:** Decision Memory and the Command Center govern cross-system names. Marketing Foundation v1.1 and Minimum Selection Doctrine v1.1 govern the Phase 1 strategy detail.

**SCOPE BOUNDARY:** this is a strategy specification only — question groups, flow logic, scoring, disqualification/routing, result-page content boundaries, CTA routing, GHL field spec, and valuation methodology. It is not code, not a rebuild instruction, and does not touch, edit, or reference-edit any file under `_BACKUPS\ECOS_BACKUPS\DIAGNOSTIC_APP_RECOVERY_LIVE_v2.3_2026-07-12\` or the live diagnostic build. A later, separate implementation phase builds from this document.
...
**Business value:** this is the offer bridge specified in `10_MARKETING_FOUNDATION_v1.1.md` §10 — *Ad or content → Second Property Readiness Check™ → result → appropriate CTA → Next Move Strategy Session™.* It is the single entry point behind both campaign doors (§2 of the Marketing Foundation master: "one Second Property Readiness Check™, one GHL contact database, one pipeline"). Its job is to qualify silently, teach the category vocabulary (Move, Route, Exit Before Entry™) as the respondent answers, and route every lead — qualified or not — to a useful next step.
...
**Decision: REPLACE (R-1).** Not kept in parallel and not merged as-is. The route naming has moved on (U-3: “Keep and Add / Sell and Rebuild / Strengthen One First” retires the prior scheme), and the two-door campaign architecture needs a Readiness Check that branches meaningfully by door. The existing live tool uses one linear path for everyone. The booked session remains the Next Move Strategy Session™.
...
| Host | GHL-native page (canonical) + a superseded parallel Apps Script deployment (see `_BACKUPS\ECOS_BACKUPS\DIAGNOSTIC_APP_RECOVERY_LIVE_v2.3_2026-07-12\_INDEX.md`) | Not specified here — hosting/build is implementation-phase scope |
...
**UI/UX improvement requirement (build note, not designed here):** the existing tool's known UX debt — mobile scroll/centering fixes needed across v2.0–v2.3, multiple incremental patches to get contrast and Back-button behaviour right (see `_BACKUPS\ECOS_BACKUPS\DIAGNOSTIC_APP_RECOVERY_LIVE_v2.3_2026-07-12\_INDEX.md`) — is not something to inherit by default. The later implementation phase must treat "meaningfully better UI/UX than the existing tool" as a build requirement, evaluated against the shipped `index_LIVE_v2.3.html` as the baseline to beat. This document does not specify pixel-level design; that is the implementation phase's job.
...
The branch fires after S3, using S1 + S3 together (S1 alone is ambiguous for EC owners, since Door A's audience includes "suitable EC owners" while Door B's audience is specifically HDB/EC owners deciding keep/sell/upgrade — see `10_MARKETING_FOUNDATION_v1.1.md` §2):
...
**Explicitly distinct from `11_MINIMUM_SELECTION_DOCTRINE_v1.1.md` §8's full selection scoring** (Plan Fit / Entry Position / Demand Growth / Exit Depth / Supply & Liquidity / Holding Strength / Unit Usability, with Strong Fit/Conditional Fit/Not Recommended conclusions). That scoring is post-engagement, property-specific, and advisor-judgement-weighted. This Readiness Score is a lightweight, entry-funnel signal only.
...
Uses only CTAs already locked in `10_MARKETING_FOUNDATION_v1.1.md` §9 — no invented CTA anywhere in this flow.
...
| Proceed lead grade | Book My Next Move Strategy Session |
...
- **The actual code or site rebuild.** This document specifies what a later implementation phase builds; it contains no code and does not touch `_BACKUPS\ECOS_BACKUPS\DIAGNOSTIC_APP_RECOVERY_LIVE_v2.3_2026-07-12\` or the live diagnostic.
...
- **The Doctrine's full post-engagement selection scoring and Final Selection Pack** (`11_MINIMUM_SELECTION_DOCTRINE_v1.1.md` §§8, 11) — this document's scoring (§6) is a distinct, lighter, marketing-facing signal only.

---

## 14. Next step

Per `10_MARKETING_FOUNDATION_v1.1.md` §14, this closes Phase 1A. Phase 1B (Door A / Door B landing-page wireframes) is next, followed by Phase 1C (first creative test packs) and Phase 1D (proof system).
