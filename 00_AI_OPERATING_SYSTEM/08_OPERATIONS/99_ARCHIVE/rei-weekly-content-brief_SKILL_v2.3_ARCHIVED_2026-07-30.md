> **ARCHIVED 2026-07-30 (same day as v2.3's creation) — superseded by `08_OPERATIONS/03_AUTOMATIONS/rei-weekly-content-brief_SKILL.md` v2.4.** Kept verbatim as rollback/history per AI OS file rule. v2.3 scoped full production to the Rank 1 angle only, with Ranks 2-5 as briefs — a Claude-proposed default flagged for confirmation, not yet approved by Edmund. Edmund confirmed the opposite: all 5 ranked angles get full production every week. v2.4 removes the Rank-1/Ranks-2-5 split accordingly.
...
Date: 2026-07-30
Supersedes: v2.2 (2026-07-30, archived at `08_OPERATIONS/99_ARCHIVE/rei-weekly-content-brief_SKILL_v2.2_ARCHIVED_2026-07-30.md`, same-day supersession after Edmund's follow-up). Changes: (1) sources point to the new expandable `06_KNOWLEDGE_VAULT/01_MARKET_INTELLIGENCE/CONTENT_SOURCE_WATCHLIST.md` instead of a hardcoded list; (2) long-form video adopts `REI_Video_Production_Foundation_v1.2.md` Format 3B (20–30 min) in place of the generic invented Format A/B/C; (3) every Rank-1 output is full word-for-word production, not an outline — script, article, carousel copy, single-image copy, per-platform social posts, WhatsApp segments, all complete; (4) every claim/segment carries an inline source citation for cross-reference; (5) images this automation cannot render get a ready-to-paste AI image-generation prompt instead — minimum 5 slides per carousel; (6) Ranks 2–5 get a strong angle brief, not full production, to keep weekly output volume realistic — see Rank 1 vs Ranks 2-5 rule below.
Sources: rei-weekly-content-brief_SKILL.md v2.2 (archived); REI_Video_Production_Foundation_v1.2.md (Format 3B + clip-extraction guide); CONTENT_SOURCE_WATCHLIST.md v1.0; 02_POSITIONING_AND_IP (Second Property Ladder spine, for business-blueprint synthesis); 2026-07-30 follow-up decision (see 04_DECISION_MEMORY.md, Decision 112).
-->

# REI WEEKLY CONTENT ENGINE — ARTICLES + YOUTUBE, ONE RANKED OUTPUT, FULL PRODUCTION
## Version: 2.3 (Monday 8:45 AM — full word-for-word output, not outlines)

**Task ID:** rei-weekly-content-brief
**Schedule:** Every Monday, 8:45 AM Singapore time
**Scheduler:** Claude Code scheduled tasks
**Execution Type:** One unified scan across articles + YouTube → one ranked angle list → Rank 1 gets full camera/publish-ready production, Ranks 2-5 get strong escalation-ready briefs

---

## WHY THIS VERSION CHANGED (from v2.2, same day)

v2.2 still produced outlines and concepts, not finished assets, and hardcoded a short source list. Edmund's follow-up made three things explicit: (1) the source list was always meant as examples to expand from, not a ceiling; (2) every output — script, article, post, image — must be the actual finished word-for-word/ready-to-use version, synthesized across everything gathered that week to produce the single highest-converting, attention-grabbing, scroll-stopping version of the angle, not a starting draft; (3) long-form YouTube should run 20–30 minutes and be built so 3 short-form pieces are cut from the same recording, not scripted four separate times. This version implements all three, and pulls in the actual validated script framework (`REI_Video_Production_Foundation_v1.2.md`, Format 3B) instead of the ad-hoc Format A/B/C this file previously invented.
...
## RANK 1 vs RANKS 2-5 — WHAT GETS BUILT

**Rank 1 gets full production** (everything in Phase C below, complete, word-for-word, publish-ready).

**Ranks 2–5 get a strong angle brief only** — not full scripts: the angle summary, the sharpest hook line, the one stat/data point it hangs on, why it ranked, source links, and which Problem Group / SI-05-or-Evergreen bucket it fits. This keeps weekly output realistic — one fully produced angle is more than most weeks can film/publish anyway — while keeping 4 more angles ready to escalate to full production the moment one is chosen (next week, or sooner on request via the on-demand mode below).

---

## PHASE C: FULL PRODUCTION FOR RANK 1 — WORD-FOR-WORD, NOT OUTLINES

Every asset below must be the finished version — synthesized across every source gathered for this angle this week, built to be the single highest-converting, attention-grabbing, scroll-stopping version of the angle, not a first draft. Every distinct claim or segment carries an inline citation — `[Source: <url>, accessed <date>]` — so it can be cross-referenced against the original later. This is not optional: it is what keeps multi-source synthesis honest and keeps this from becoming a rewrite of any single outlet's proprietary work (see Attribution Rule below).
...
**Weekly dated brief** (Rank 1 full production + Ranks 2-5 briefs, all in one file) → `H:\Shared drives\00_E.C.O.S\00_AI_OPERATING_SYSTEM\06_KNOWLEDGE_VAULT\01_MARKET_INTELLIGENCE\[WEEK] REI Weekly Brief — YYYY-MM-DD.md`.
...
4. Rank 1 = full production, word-for-word, every asset type in Phase C. Ranks 2-5 = brief only, not full scripts — do not skip this split; it is what keeps weekly runtime realistic.
...
## END REI WEEKLY CONTENT ENGINE v2.3
